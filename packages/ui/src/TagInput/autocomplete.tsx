...
export const Autocomplete: React.FC<AutocompleteProps> = ({
  ...
  return (
    <Command className={cn("h-full w-full", classStyleProps?.command)}>
      <Popover open={isPopoverOpen} onOpenChange={handleOpenChange}>
        ...
        >
          <CommandList className={cn(classStyleProps?.commandList)}>
            ...
            <CommandGroup
              heading="Suggestions"
              className={cn("overflow-y-auto", classStyleProps?.commandGroup)}
            >
              {autocompleteOptions.map((option) => (
                <CommandItem
                  key={option.id}
                  className={cn("cursor-pointer", classStyleProps?.commandItem)}
                >
                  <div
