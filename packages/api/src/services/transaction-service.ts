...
export class TransactionService {
  ...
  async getReceiptInfo(transactionId: number) {
    ...
    const studentContact = await this.db.studentContact.findMany({
      include: {
        contact: true,
      },
      where: {
  ...
  async getTransactionStats({
    ...
    const currentFees = await this.db.fee.aggregate({
      _sum: {
        amount: true,
      },
      where: {
    ...
    const totalCompleted = await this.db.transaction.aggregate({
      _sum: {
        amount: true,
      },
      where: whereClause,
    });
    const totalInProgress = await this.db.transaction.aggregate({
      _sum: {
        amount: true,
      },
      where: {
    ...
    const totalDeleted = await this.db.transaction.aggregate({
      _sum: {
        amount: true,
      },
      where: {
