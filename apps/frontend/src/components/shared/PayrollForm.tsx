...
export const CNPS_RATE = 0.04;
export const CNSS_RATE = 0.01;
...
export interface PayrollFormDefaultValues {
  staffId?: string;
  month?: string;
  year?: string;
  grossSalary?: string;
...
interface CalculatedDeductions {
  cnps: number;
  cnss: number;
  advance: number;
  other: number;
...
interface PayrollFormProps {
  defaultValues?: PayrollFormDefaultValues;
  initialCalculated?: CalculatedDeductions;
  onSave: (data: PayrollSaveData) => void;
  isSaving: boolean;
}
...
export function PayrollForm({
  ...
  const trpc = useTRPC();
  const [calculatedDeductions, setCalculatedDeductions] =
    useState<CalculatedDeductions | null>(initialCalculated ?? null);
  ...
  const form = useForm({
    defaultValues: {
      ...
      grossSalary: defaultValues?.grossSalary ?? "",
      advance: defaultValues?.advance ?? "",
      otherDeductions: defaultValues?.otherDeductions ?? "",
      paymentMode: defaultValues?.paymentMode ?? "virement",
  ...
  function handleSave() {
    ...
    const monthIndex = MONTHS.indexOf(values.month as (typeof MONTHS)[number]);
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle>Paramètres Configurables Appliqués</CardTitle>
        ...
        <div className="space-y-6 lg:col-span-2">
          <Card>
            ...
            <CardContent className="flex flex-col gap-6">
              ...
              {selectedStaff ? (
                <div className="bg-muted text-muted-foreground flex items-center justify-between rounded-lg p-2 text-center text-xs">
                  <div>
                    Poste:{" "}
                ...
                <div className="bg-muted text-muted-foreground rounded-lg p-2 text-center text-xs">
