      ...
      <div className="flex w-full items-center">
        <Reorder.Item
          ...
          className={cn(
            ...
            "h-full rounded-md border bg-muted/50 text-muted-foreground",
                ...
                {!isExpanded ? (
                  ...
                  >
                    ...
                    <Checkbox
                      ...
                      className="0 ml-3 h-5 w-5 rounded-md data-[state=checked]:bg-black data-[state=checked]:text-red-200"
                    />
                    {/* List Order */}
                    <p className="pl-1 font-mono text-xs">{order + 1}</p>
                    ...
                    >
                      <h4
                        className={cn(
                          ...
                          item.checked && "text-destructive",
                        )}
                      >
                        {item.checked ? "Delete" : ` ${item.name}`}
                      </h4>
                    </motion.div>
                  </motion.div>
                ) : null}
        ...
        <AnimatePresence mode="popLayout">
          {item.checked ? (
            ...
            >
              ...
              >
                <Trash2 className="h-4 w-4 fill-red-400/60 text-red-400 transition-colors duration-150" />
              </button>
            </motion.div>
          ) : null}
        </AnimatePresence>
          ...
          <AnimatePresence>
            {items.map((item, index) =>
              renderItem(item, index, onCompleteItem, (id: number) => {
                ...
                deleteClassroomLevelMutation.mutate(item.id);
              }),
            )}
          </AnimatePresence>
        </Reorder.Group>
