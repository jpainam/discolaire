...
const querySchema = z.object({
  ...
  doc: z.enum(["subject", "program"]).default("program"),
});

export async function GET(
  request: NextRequest,
  { params }: { params: { subjectId: string; id: string } },
) {
  ...
  if (!id || !subjectId) {
    return NextResponse.json({ message: "No subjectId provided", status: 400 });
  }
...
async function toExcel({
  ...
  const filename = `Programmes-${classroom.name}.xlsx`;
